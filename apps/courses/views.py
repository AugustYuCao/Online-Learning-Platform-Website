...
class VideoView(LoginRequiredMixin, View):    #登陆才能访问
    login_url = "/login/"
    ...
    def get(self, request, course_id, video_id,  *args, **kwargs):
        ...
        #查询用户是否已经关联了该课程
        user_courses = UserCourse.objects.filter(user=request.user, course=course)    #用户能进入说明已经处于登陆状态，不用再检查
        #关联用户和课程
        if not user_courses:
...
class CourseListView(View):
    def get(self, request, *args, **kwargs):
        ...
        s_type = "course"
        if keywords:
            all_courses = all_courses.filter(Q(name__icontains=keywords)|Q(desc__icontains=keywords)|Q(detail__icontains=keywords))    #i表示忽略大小写


        #课程排序
        sort = request.GET.get("sort", "")    #取前端返回值
        if sort == "students":
        ...
        p = Paginator(all_courses, per_page=3, request=request)    #每页3条
        courses = p.page(page)
